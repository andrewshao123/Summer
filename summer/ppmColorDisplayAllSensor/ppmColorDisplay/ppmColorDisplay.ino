...
int delay1 = 500; // delay for half a second
...
int average = 0;
//USB port
int resistanceRange[] = {1,10}; //range for gas sensor value***change this value to change the upper/lower bound resistance range
//Battery
...
float calcVoltageDust = 0;
float dustDensityDust = 0;

float calcVoltageNOx = 0;
float dustDensityNOx = 0;

float calcVoltageO3 = 0;
float dustDensityO3 = 0;

/*O3 sensor*/
int gasSensorO3 = 1; // select analog pin for gasSensor
int valO3 = 0; // variable to store the value coming from the sensor
...
void loop(){
  ...
  calcVoltageNOx = val * (3.3 / 1024);
  dustDensityNOx = 0.17 * calcVoltageNOx - 0.1;

//  delay(delay1);

  //Serial.print("val: ");
  //Serial.print(val);
  Serial.print(" NOx density: ");
  Serial.print( dustDensityNOx );
  ...
  // recover voltage
  calcVoltageDust = voMeasured * (3.3 / 1024);
  dustDensityDust = 0.17 * calcVoltageDust - 0.1;
  //Serial.print(" Raw: ");
  //Serial.print(voMeasured);

  Serial.print(" Dust Density: ");
  Serial.print(dustDensityDust);

  /*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$O3 sensor$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
  valO3 = analogRead(gasSensorO3); // read the value from the pot
  calcVoltageO3 = valO3 * (3.3 / 1024);
  dustDensityO3 = 0.17 * calcVoltageO3 - 0.1;


  //Serial.print(" valO3: ");
  //Serial.println(valO3);
  Serial.print(" O3 density ");
  Serial.print(dustDensityO3);

  delay(delay1);
  
  AQHI = (1000/10.4)*((exp(0.000537*dustDensityO3*100)-1) + (exp(0.000871*dustDensityNOx * 100)-1) + (exp(0.000487*dustDensityDust*100)-1));
  Serial.print(" AQHI: ");
  Serial.print(AQHI);
  Serial.print(" int AQHI: ");
  Serial.println(int(AQHI));
...
//  }
  if (AQHI != 0) {
    if (AQHI >= resistanceRange[0] && AQHI <= resistanceRange[1]){//filter out the edge values such as if the values are out of the range or if the value is 0    
      colorIndex = custom_map(AQHI);//convert the color value to the one that is mapped to the led
      Serial.print("colorindex: ");
      Serial.println(colorIndex);
    }
    else if(AQHI > resistanceRange[1]){
      //colorIndex = 255;
      colorIndex = 1400;
    }
    else if(AQHI < resistanceRange[0]){
      colorIndex = 255;
      //colorIndex = 1400;
    }
    Display(colorIndex);
